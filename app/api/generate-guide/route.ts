...
[문서 구조]
확인된 작업 유형만 섹션으로 작성한다. 해당 기능에 없는 작업은 섹션을 만들지 않는다.
용어 정의·상태값 표는 문서 초반에 배치하지 않는다. 필요하면 해당 섹션 안에서 한두 줄로 설명하거나 문서 하단 부록으로 분리한다.
...
### 작업 순서
[이미지 삽입 영역]
1. 운영자가 화면에서 해야 할 행동을 "~합니다" 형태로 설명한다.
[이미지 삽입 영역]
2. 다음 단계를 설명한다.
...
| 항목 | 필수 여부 | 입력/선택 기준 | 저장 전 정상 기준 | 비고 |
|---|---|---|---|---|

### 저장 전 확인 사항
예/아니오로 판단 가능한 항목만 작성한다.
"정책에 맞게 확인합니다", "올바르게 설정합니다" 같은 추상 문장은 쓰지 않는다.
- [ ] 확인 항목 1
- [ ] 확인 항목 2

### 저장 후 결과
- 저장 후 어드민에서 어떻게 바뀌는지 설명한다.
...
## 검수 필요 항목
본문에는 넣지 않는다. Figma·정책·스펙에서 확인되지 않은 내용만 이 표에 기록한다.
| 항목 | 확인이 필요한 이유 | 확인 대상자 |

[불명확한 내용 처리 원칙]
아래 경우는 본문에 넣지 않는다. 문서 마지막 "검수 필요 항목" 표에만 기록한다.
...
- 삭제, 복구, 수동 상태 변경, 재개 등 근거가 불명확한 기능 — 일반적인 어드민 기능이라고 추측해서 넣지 않는다.
...
제공된 Figma 화면설계서, 정책, 스펙을 기반으로 운영자가 실제 업무에 사용할 수 있는 운영 가이드 초안을 작성합니다.
목표는 기능 설명서가 아니라, 운영자가 이미지를 보면서 그대로 따라 할 수 있는 쉬운 실행 가이드입니다.

[작성 관점]
- 운영자가 실제 작업하는 순서대로 작성합니다: 어디로 들어가서 → 무엇을 입력하고 → 저장 전에 무엇을 확인하고 → 저장 후 어떻게 되는지.
- 화면/정책/스펙에서 확정할 수 없는 내용은 본문에 쓰지 않습니다.
- 삭제, 복구, 재개, 수동 상태 변경처럼 근거가 불명확한 기능은 일반적인 어드민 기능이라고 추측해서 넣지 않습니다.
