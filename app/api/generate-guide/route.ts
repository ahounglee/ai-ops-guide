...
2. 디스크립션에 언급된 모든 입력 필드는 빠짐없이 입력 항목 표에 독립 행으로 추가합니다. 단순화나 요약을 위해 생략하지 않습니다.
   - 메인 항목에 종속된 조건 필드(최솟값, 최댓값, 할인 조건 세부값, 수량 제한 등)도 반드시 별도 행으로 작성합니다.
   - 예: 정액 할인 조건 전체, 정률 최대 할인 금액, 적용 최소 금액, 총 발급 수량 등은 각각 독립 행으로 기술합니다.
   - 선택지별로 달라지는 모든 하위 항목을 빠짐없이 기술합니다. 하나라도 누락되면 안 됩니다.
   - "기술 스펙" 또는 "개발 메모"로 판단되더라도 운영자가 직접 입력하는 필드라면 반드시 포함합니다.
3. 입력 자료에 대한 메타 설명을 추가하지 않습니다. ("파악하기 어렵습니다", "정보가 부족합니다" 등 금지)
4. 출력물은 반드시 운영 가이드 형식이어야 합니다. 요약 정리, 분류표, 설명문 형태로 작성하지 않습니다.`;
...
- 입력 필드가 있는 단계에는 값의 범위·제한 조건·계산 규칙을 괄호 안에 함께 표기합니다.
  예: `할인율`을 입력합니다. (0.01~100 사이 숫자 입력)
  예: `쿠폰 발급 장수`를 입력합니다. (최대 발급 장수 제한 없음)

**입력 항목**
| 항목 | 필수 여부 | 입력 기준 | 비고 |
|---|---|---|---|
- 자료에서 확인된 항목을 빠짐없이 작성합니다. 임의로 생략하지 않습니다.
- 필수 여부는 "필수" 또는 "선택"으로 표기합니다.
- 특정 조건에서만 나타나는 항목은 필수 여부 칸에 "[A] 선택 시 필수" 또는 "[A] 선택 시 표시"로 명시합니다.
- 선택지가 있는 항목(정액/정률 등)은 각 선택지마다 달라지는 하위 항목을 모두 별도 행으로 작성합니다.
- 비고란에는 단순 입력 방법이 아니라, 정책 문서에 명시된 값의 범위·제한 조건·계산 규칙·소수점 처리 방식 등을 반드시 포함합니다. 운영자가 "얼마를 입력할 수 있고, 어떤 제한이 걸리는지"를 한눈에 파악할 수 있어야 합니다. (예: "0.01~100 입력 가능", "최대 N자", "소수점 이하 버림")
